...
// Help performance of the server sending data to the client by marking this machine as
// little-endian
...
typedef enum rmtSampleFlags {
    ...
    // Set this flag on any of your root samples so that Remotery will assert if it ends up *not*
    // being the root sample. This will quickly allow you to detect Begin/End mismatches causing a
    // sample tree imbalance.
    RMTSF_Root = 4,
} rmtSampleFlags;
...
#define rmt_DestroyGlobalInstance(rmt) RMT_OPTIONAL(RMT_ENABLED, _rmt_DestroyGlobalInstance(rmt))

#define rmt_SetGlobalInstance(rmt) RMT_OPTIONAL(RMT_ENABLED, _rmt_SetGlobalInstance(rmt))

#define rmt_GetGlobalInstance() RMT_OPTIONAL_RET(RMT_ENABLED, _rmt_GetGlobalInstance(), NULL)

#define rmt_SetCurrentThreadName(rmt) RMT_OPTIONAL(RMT_ENABLED, _rmt_SetCurrentThreadName(rmt))
...
#define rmt_IterateNext(iter) RMT_OPTIONAL_RET(RMT_ENABLED, _rmt_IterateNext(iter), RMT_FALSE)
...
#define rmt_SampleGetName(sample) RMT_OPTIONAL_RET(RMT_ENABLED, _rmt_SampleGetName(sample), NULL)
...
#define rmt_SampleGetStart(sample) RMT_OPTIONAL_RET(RMT_ENABLED, _rmt_SampleGetStart(sample), 0LLU)

#define rmt_SampleGetTime(sample) RMT_OPTIONAL_RET(RMT_ENABLED, _rmt_SampleGetTime(sample), 0LLU)
...
typedef struct rmtCUDABind {
    ...
    // NOTE: These are named differently to the CUDA functions because the CUDA API has a habit of
    // using macros to point function calls to different versions, e.g. cuEventDestroy is a macro
    // for cuEventDestroy_v2.
    void* CtxSetCurrent;
    void* CtxGetCurrent;
    void* EventCreate;
    void* EventDestroy;
...
#define rmt_EndCUDASample(stream) RMT_OPTIONAL(RMT_USE_CUDA, _rmt_EndCUDASample(stream))

#define rmt_BindD3D11(device, context) RMT_OPTIONAL(RMT_USE_D3D11, _rmt_BindD3D11(device, context))
...
#define rmt_EndOpenGLSample() RMT_OPTIONAL(RMT_USE_OPENGL, _rmt_EndOpenGLSample())

#define rmt_BindMetal(command_buffer) RMT_OPTIONAL(RMT_USE_METAL, _rmt_BindMetal(command_buffer));
